...
function checkboxList(items = [], checked = false) {
  ...
  return items.length
    ? items.map((item) => `- [${isChecked(item) ? "x" : " "}] ${item}`).join("\n")
    : "- [ ] none declared";
}
