...
export function createWorld() {
  ...
  return {
    addEntity() {
      ...
      entities.add(entity);
      record("entity", { action: "add", entity });
      return entity;
    },
    ...
    removeEntity(entity) {
      ...
      record("entity", { action: "remove", entity });
      return true;
    },
    ...
    emit(event, payload) {
      const queue = ensureEventQueue(event);
      queue.push(payload);
...
export function createScheduler() {
  ...
  const scheduler = {
    ...
    run(world, hooks = {}) {
      if (!world || typeof world.clearAllEvents !== "function" || typeof world.drainJournal !== "function") {
        throw new TypeError("Scheduler requires a world created by createWorld().");
      }

      for (const phaseName of phases) {
        if (typeof hooks.onPhaseStart === "function") {
          hooks.onPhaseStart(phaseName, world);
        }
        ...
        if (typeof hooks.onPhaseEnd === "function") {
          hooks.onPhaseEnd(phaseName, world);
        }
      }
      ...
      if (typeof hooks.onDrain === "function") {
        hooks.onDrain({ world, journal, phases: phases.slice() });
      }
