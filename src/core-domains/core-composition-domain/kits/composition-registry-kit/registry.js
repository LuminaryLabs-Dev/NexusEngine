...
function createCoreCatalogBody() {
  ...
  const legacyKits = CORE_KITS.map(([id, domain, domainPath, parentDomainPath, exportName, requires = [], extraProvides = []]) => ({
    id,
  ...
  const domainKitIds = new Set(CORE_DOMAIN_CATALOG.kits.map((kit) => kit.id));
  for (const kit of legacyKits) {
    if (domainKitIds.has(kit.id)) {
      throw new TypeError(`Core Kit ${kit.id} appears in both the domain-owned and legacy catalogs.`);
    }
  }
  const domainOwnedKits = CORE_DOMAIN_CATALOG.kits.map((kit) => ({
    ...clone(kit),
    apiVisibility: "public",
    source: {
      registryId: sourceId,
  ...
  const kits = [...legacyKits, ...domainOwnedKits];
  const pathParents = new Map([["n:graphics", null]]);
  for (const kit of legacyKits) pathParents.set(kit.domainPath, kit.parentDomainPath);
  const legacyDomains = [...pathParents.entries()].map(([domainPath, parentDomainPath]) => ({
  ...
  const manifestPaths = new Set(CORE_DOMAIN_CATALOG.domains.map((domain) => domain.domainPath));
  const duplicatePath = legacyDomains.find((domain) => manifestPaths.has(domain.domainPath));
  if (duplicatePath) {
    throw new TypeError(`Core domain path ${duplicatePath.domainPath} appears in both the domain-owned and legacy catalogs.`);
  }
  const domains = [
    ...legacyDomains,
    ...CORE_DOMAIN_CATALOG.domains.map((domain) => ({
      ...clone(domain),
      sourceRegistryId: sourceId
    }))
  ];
