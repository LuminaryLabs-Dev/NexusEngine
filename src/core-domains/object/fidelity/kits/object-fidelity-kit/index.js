    ...
    async prepare({ object, requirement, buildId }) {
      ...
      return {
        form: {
          ...
          layers,
          metadata: { source: "object", objectContentHash: object.contentHash }
        }
      };
    }
  };
...
export function createObjectFidelityKit(config = {}) {
  return defineDomainServiceKit({
    manifestId: "object-fidelity-kit",
    id: config.id ?? "object-fidelity-kit",
    ...
    createApi({ engine, world }) {
      ...
      function currentObject(build) {
        return engine.n?.object?.get?.(build.objectId) ?? null;
      }
      ...
      async function requestBuild(input = {}) {
        ...
        const object = engine.n?.object?.get?.(objectId);
...
export default createObjectFidelityKit;
