...
import { createObjectDescriptor } from "../../../../contracts/object-descriptor.js";
import { createVegetationInstanceDescriptor } from "../../kits/object-vegetation-kit/contracts.js";
...
export function vegetationSpeciesToObjectDescriptor(speciesInput, options = {}) {
  ...
  return createObjectDescriptor({
    ...
    material: reference(options.material ?? species.references?.material, "graphics", `${species.id}:materials`, { speciesId: species.id }),
    collision: reference(options.collision ?? species.references?.collision, "physics", `${species.id}:collision`, { speciesId: species.id }),
    lod: reference(options.fidelity ?? species.references?.fidelity, "core-object-fidelity", `${species.id}:fidelity`, { speciesId: species.id }),
    capture: reference(options.capture ?? species.references?.capture, "capture", `${species.id}:capture`, { speciesId: species.id }),
...
export function createVegetationObjectBridgeKit(config = {}) {
  return defineDomainServiceKit({
    manifestId: "object-vegetation-bridge-kit",
    id: config.id ?? "object-vegetation-bridge-kit",
    ...
    createApi({ engine }) {
      ...
      return {
        ...
        registerSpeciesObject(speciesOrId, options = {}) {
          return object.register(this.toObjectDescriptor(speciesOrId, options));
        },
        registerInstanceObject(instanceOrId, options = {}) {
          return object.register(this.toInstanceObjectDescriptor(instanceOrId, options));
        }
      };
    }
  });
