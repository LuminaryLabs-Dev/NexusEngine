...
export function createObjectShapeKit(config = {}) {
  return defineDomainServiceKit({
    manifestId: "object-shape-kit",
    id: config.id ?? "object-shape-kit",
    domain: "object-shape",
    ...
    createApi({ engine, world }) {
      ...
      function isCurrent(job) {
        const object = engine.n?.object?.get?.(job.objectId);
      ...
      const api = {
        ...
        registerSource(input) {
          ...
          const object = engine.n?.object?.get?.(source.objectId);
...
export default createObjectShapeKit;
