...
export function createReflectionKit(config = {}) {
  ...
  return createDomainKit({
    ...config,
    manifestId: "reflection-descriptor-kit",
    id: config.id ?? "reflection-descriptor-kit",
    domain: "reflection",
    domainPath: config.domainPath ?? "n:presentation:graphics",
    parentDomainPath: config.parentDomainPath ?? "n:presentation",
    apiName: config.apiName ?? "reflection",
    ...
    requires: unique(["n:presentation:graphics", ...(config.requires ?? [])]),
...
export default createReflectionKit;
