...
import { createCoreMcpDomain, defineMcpProvider } from "../index.js";
...
await assert.rejects(
  engine.n.coreMcp.callTool("write-state", { value: 1 }),
  /requires explicit authorization/
);
await assert.rejects(
  engine.n.coreMcp.callTool("write-state", { value: "bad" }, { authorize: () => true }),
  /failed validation/
);
assert.equal(
  (await engine.n.coreMcp.callTool("write-state", { value: 2 }, { authorize: () => true }))
...
for (const [id, handler] of invalidOutputs) {
  ...
  await assert.rejects(
    engine.n.coreMcp.callTool(`invalid-output-${id}`),
