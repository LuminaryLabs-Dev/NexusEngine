...
export function createStartupDescriptor(state) {
...
export function createStartupKit(config = {}) {
  const historyLimit = Math.max(8, Math.floor(Number(config.historyLimit) || 128));
  const initial = createInitialState(config);

  return defineDomainServiceKit({
    manifestId: "runtime-startup-kit",
    id: config.id ?? "runtime-startup-kit",
    domain: "startup",
    domainPath: config.domainPath ?? "n:runtime:startup",
    parentDomainPath: config.parentDomainPath ?? "n:runtime",
    apiName: config.apiName ?? "startup",
    version: CORE_STARTUP_VERSION,
    ...
    createApi({ engine, world }) {
      ...
      const emitState = (next, event, payload = {}) => {
        ...
        world.emit(event, { state: clone(next), descriptor: createStartupDescriptor(next), ...clone(payload) });
        return clone(next);
      };
      ...
      function launch(request = {}) {
        ...
        return createStartupDescriptor(next);
      }
      ...
      function enter(payload = {}) {
        ...
        if (currentState.launch.status === "ready" && currentState.playable) {
          return createStartupDescriptor(currentState);
        }
        const state = assertActive();
        const summary = summarize(state);
        ...
        return createStartupDescriptor(getStateInternal());
      }
      ...
      function cancel(reason = "cancelled") {
        const state = getStateInternal();
        if (TERMINAL_STATUSES.has(state.launch.status)) return createStartupDescriptor(state);
        update({
      ...
      const api = {
        ...
        getDescriptor: () => createStartupDescriptor(getStateInternal()),
        getPreparation(id) {
...
export default createStartupKit;
