...
export function createDataKit(config = {}) {
  const customCreateApi = config.createApi;
  const customInstall = config.install;
  const apiName = config.apiName ?? "data";

  return createDomainKit({
    ...config,
    manifestId: "runtime-data-kit",
    id: config.id ?? "runtime-data-kit",
    domain: "data",
    domainPath: config.domainPath ?? "n:runtime:data",
