...
export function createWeatherKit(config = {}) {
  const userInstall = config.install;
  return createDomainKit({
    ...config,
    manifestId: "weather-state-kit",
    id: config.id ?? "weather-state-kit",
    domain: "weather",
    domainPath: config.domainPath ?? "n:world:weather",
    parentDomainPath: config.parentDomainPath ?? "n:world",
    apiName: config.apiName ?? "weather",
    requires: ["n:world", ...(config.requires ?? [])],
...
export default createWeatherKit;
