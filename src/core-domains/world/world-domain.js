...
function featureFamilyConfig(config, name) {
  if (config.featureFamilies?.[name] !== undefined) return config.featureFamilies[name];
  return {};
}

export function createWorldDomain(config = {}) {
  const userInstall = config.install;
  return createDomainKit({
    ...config,
    manifestId: "world-state-kit",
    id: config.id ?? "world-state-kit",
    domain: "world",
    domainPath: config.domainPath ?? "n:world",
    apiName: config.apiName ?? "world",
      ...
      if (config.childDomains !== false) {
        ...
        if (config.features !== false) {
          ...
          if (config.featureFamilies !== false) {
            const landforms = featureFamilyConfig(config, "landforms");
            const hydrology = featureFamilyConfig(config, "hydrology");
            const ecology = featureFamilyConfig(config, "ecology");
            const settlement = featureFamilyConfig(config, "settlement");
