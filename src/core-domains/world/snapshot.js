...
export function createWorldRecordSnapshot(record, options = {}) {
...
export function createWorldSnapshot(state, runtimeWorlds = new Map()) {
  const worlds = {};
  for (const [worldId, record] of Object.entries(state?.worlds ?? {})) {
    ...
    worlds[worldId] = createWorldRecordSnapshot(record, providerData);
  }
