...
export function createSceneKit(config = {}) {
  const startState = createInitialSceneState(config);
  const userCreateApi = config.createApi;

  return createDomainKit({
    ...config,
    manifestId: "scene-lifecycle-kit",
    id: config.id ?? "scene-lifecycle-kit",
    domain: "scene",
    domainPath: config.domainPath ?? "n:world:scene",
    parentDomainPath: config.parentDomainPath ?? "n:world",
    apiName: config.apiName ?? "scene",
      ...
      const api = {
        ...
        requestTransition(request = {}) {
          if (!request || typeof request !== "object" || Array.isArray(request)) {
            throw new TypeError("scene.requestTransition expects a request object.");
          }
          const current = state();
