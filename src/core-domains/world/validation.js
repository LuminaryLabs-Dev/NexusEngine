...
export function validateWorldProviderSet(providers = []) {
  ...
  for (const provider of providers) {
    ...
    if (typeof provider?.prepareCell !== "function") issues.push(`provider:${provider?.id ?? "unknown"}:missing-prepare`);
...
export function validateCoreWorldState(state) {
  ...
  issues.push(...inspectPortableValue(state, { path: "worldState" }).issues);
  return { valid: issues.length === 0, issues };
}
