...
import { createEngineRegistrySnapshot } from "./registry.js";
import {
...
export function createCompositionKit(config = {}) {
  const customCreateApi = config.createApi;
  const customInstall = config.install;
  const apiName = config.apiName ?? "composition";

  return createDomainKit({
    ...config,

    manifestId: "composition-registry-kit",

    id: config.id ?? "composition-registry-kit",

    domainPath: config.domainPath ?? "n:composition",
    domain: "composition",
    apiName,
    ...
    createApi(context) {
      const initialRegistry = config.registry === false
        ? { kits: [], domains: [], recipes: [] }
        : config.registry ?? createEngineRegistrySnapshot();
