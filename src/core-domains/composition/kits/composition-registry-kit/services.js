...
function configSnapshot(config = {}) {
  if (config.schema) return normalizeRegistrySnapshot(config, { allowBuiltinSources: true });
  const content = { kits: asList(config.kits), domains: asList(config.domains), recipes: asList(config.recipes) };
  const integrity = hashRegistryValue(content);
  return normalizeRegistrySnapshot({
    ...
    kits: content.kits.map((kit) => ({
      ...
      source: {
        ...
        installable: false,
        ...(kit.source ?? {}),
        registryId: kit.source?.registryId ?? RUNTIME_SOURCE_ID
      }
    })),
    domains: content.domains.map((domain) => ({ ...domain, sourceRegistryId: domain.sourceRegistryId ?? RUNTIME_SOURCE_ID })),
    recipes: content.recipes.map((recipe) => ({ ...recipe, sourceRegistryId: recipe.sourceRegistryId ?? RUNTIME_SOURCE_ID }))
  }, { allowBuiltinSources: true });
}

export function createKitRegistryService(config = {}) {
  const initial = configSnapshot(config);
  let kits = new Map(); let domains = new Map(); let recipes = new Map(); let sources = new Map(); let revision = 0;
  let protectedIds = new Set(); let protectedDomainPaths = new Set();

  function ensureRuntimeSource() {
    if (!sources.has(RUNTIME_SOURCE_ID)) {
      const integrity = hashRegistryValue({ registryId: RUNTIME_SOURCE_ID, status: "metadata-only" });
      sources.set(RUNTIME_SOURCE_ID, Object.freeze({
      registryId: RUNTIME_SOURCE_ID,
      package: "runtime-registry",
      version: "0.0.0",
      sourceCommit: `builtin:${integrity.slice("sha256:".length)}`,
      integrity,
      status: "metadata-only",
      environments: Object.freeze(["browser", "node", "worker"]),
  ...
  function register(map, value, normalize, label) {
    ensureRuntimeSource();
    ...
    if (existing) throw new TypeError(`${label} id collision with different content: ${normalized.id}`);
    if (label === "Domain") {
  ...
  function snapshot() {
    return normalizeRegistrySnapshot({
      ...
      recipes: sortedValues(recipes)
    }, { allowBuiltinSources: true });
  }
  function loadSnapshot(value = {}) {
    const normalized = normalizeRegistrySnapshot(value, { allowBuiltinSources: true });
    kits = new Map(normalized.kits.map((record) => [record.id, record]));
    domains = new Map(normalized.domains.map((record) => [record.id, record]));
    recipes = new Map(normalized.recipes.map((record) => [record.id, record]));
    sources = new Map(normalized.sources.map((record) => [record.registryId, record]));
    revision = normalized.revision;
    protectedIds = new Set([...normalized.domains, ...normalized.kits, ...normalized.recipes].filter((record) => {
      const sourceId = record.sourceRegistryId ?? record.source?.registryId;
      return sources.get(sourceId)?.metadata?.core === true || record.metadata?.core === true;
  ...
  return Object.freeze({
    normalizeKitManifest: (value) => normalizeKitRegistryRecord(value, { defaultSourceId: RUNTIME_SOURCE_ID }),
    ...
    normalizeRecipeManifest: (value) => normalizeRecipeRegistryRecord({ ...value, sourceRegistryId: value.sourceRegistryId ?? RUNTIME_SOURCE_ID }),
    registerKit: (value) => register(kits, value, (entry) => normalizeKitRegistryRecord({
      ...entry,
      source: {
        subpath: null,
        exportName: null,
        ...
        installable: false,
        ...(entry.source ?? {}),
        registryId: entry.source?.registryId ?? RUNTIME_SOURCE_ID
      }
    }, { defaultSourceId: RUNTIME_SOURCE_ID }), "Kit"),
    registerDomain: (value) => register(domains, value, (entry) => normalizeDomainRegistryRecord({ ...entry, sourceRegistryId: entry.sourceRegistryId ?? RUNTIME_SOURCE_ID }), "Domain"),
    registerRecipe: (value) => register(recipes, value, (entry) => normalizeRecipeRegistryRecord({ ...entry, sourceRegistryId: entry.sourceRegistryId ?? RUNTIME_SOURCE_ID }), "Recipe"),
    registerRegistry(value = {}, options = {}) {
      ...
      loadSnapshot(merged);
      return { kits: clone(merged.kits), domains: clone(merged.domains), recipes: clone(merged.recipes), snapshot: clone(merged) };
    },
    ...
    removeRecipe(id) { if (protectedIds.has(String(id))) throw new TypeError(`Core recipe ${id} cannot be removed.`); const removed = recipes.delete(String(id)); if (removed) revision += 1; return removed; },
    getKit: (id) => clone(kits.get(String(id)) ?? null),
    getDomain: (id) => clone(domains.get(String(id)) ?? null),
    getRecipe: (id) => clone(recipes.get(String(id)) ?? null),
    listKits: () => sortedValues(kits),
    listDomains: () => sortedValues(domains),
    listRecipes: () => sortedValues(recipes),
    ...
    search(query = "") {
      ...
      return sortedValues(kits).filter((kit) => [kit.id, kit.domainPath, kit.apiName, ...kit.requires, ...kit.provides].filter(Boolean).some((value) => String(value).toLowerCase().includes(text)));
    },
    getState: snapshot,
...
export function createCompositionPlanningService(registry, capabilities) {
  ...
  function requestedKitIds(request = {}) {
    ...
    for (const recipeId of asList(request.recipes)) {
      const recipe = registry.getRecipe(recipeId); if (!recipe) throw new RangeError(`Unknown composition recipe: ${recipeId}`);
      for (const kitId of recipe.kits) selected.add(kitId);
      for (const domainId of recipe.domains) {
        const domain = registry.getDomain(domainId); if (!domain) throw new RangeError(`Recipe ${recipeId} references unknown domain ${domainId}.`);
        for (const kit of registry.listKits()) {
          if (kit.domainPath === domain.domainPath || kit.domainPath.startsWith(`${domain.domainPath}:`)) selected.add(kit.id);
        }
      }
  ...
  function plan(request = {}, options = {}) {
    ...
    const result = { schema: "nexusengine.composition-plan/2", requested: requestedKitIds(request), selected: selectedIds, order: [...ordered, ...unresolvedOrder], missing: dedupe(missing), rejected: dedupe(rejected), cycles, ok: missing.length === 0 && rejected.length === 0 && cycles.length === 0 };
...
export function createCompositionHealthService(registry, capabilities) {
  return Object.freeze({
    getSnapshot() {
      ...
      return { schema: "nexusengine.composition-health/2", revision: registrySnapshot.revision, kits: registrySnapshot.kits.length, domains: registrySnapshot.domains.length, recipes: registrySnapshot.recipes.length, statusCounts, missingDependencies: graph.missing.length, cycles: graph.cycles.length, healthy: graph.valid };
    }
  });
}
