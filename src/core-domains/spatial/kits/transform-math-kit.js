...
export function createTransformMathKit(config = {}) {
  return createDomainKit({
    ...config,
    manifestId: "spatial-transform-math-kit",
    id: "spatial-transform-math-kit",
    domain: "spatial-transform-math",
    domainPath: "n:spatial",
    apiName: "transformMath",
    provides: ["spatial:transform-math"],
    purpose: "Provide deterministic transform, basis, interpolation, and planar projection operations.",
    createApi: () => ({
      TAU, EPSILON, normalizeAngle, shortestAngle, clamp, clampAngle, lerp,
      lerpAngle, smoothstep, expSmoothingAlpha, vec3, add, sub, scale, dot,
      cross, lengthSq, length, normalize, forwardFromYaw, rightFromYaw,
      basisFromYaw, yawFromForward, up, directionBetween, midpoint,
      segmentLength, projectOntoPlane, planarBasisFromForward,
      wishVectorFromBasis, cameraRelativeWishVector,
      cameraRelativeWishVectorFromForward, signedAngleOnPlane
    })
  });
}
