...
function createApi(config, engine, world) {
  ...
  const physics = () => engine.n?.physics ?? null;
  ...
  return Object.freeze({
    ...
    syncArticulation(input = {}) {
      ...
      physics()?.syncArticulations?.(Object.values(state().articulations ?? {}));
      physics()?.syncConstraints?.(
    ...
    removeArticulation(id) {
      ...
      if (removed) {
        ...
        physics()?.syncArticulations?.(Object.values(articulations));
        physics()?.syncConstraints?.(Object.values(articulations).flatMap((entry) => entry.joints ?? []));
      }
      return removed;
    },
    ...
    commitFrame(input = {}) {
      const current = state();
      const physicsFrame = input.physicsFrame ?? physics()?.getFrame?.() ?? {};
    ...
    reset() {
      physics()?.syncArticulations?.([]);
      physics()?.syncConstraints?.([]);
      physics()?.submitJointMotorRequests?.([]);
...
export function createArticulatedDynamicsDomain(config = {}) {
  return defineDomainServiceKit({
    manifestId: "articulated-physics-kit",
    id: config.id ?? "articulated-physics-kit",
    domain: "articulated-dynamics",
    domainPath: "n:simulation:physics:articulated",
    parentDomainPath: "n:simulation:physics",
    apiName: config.apiName ?? "articulatedPhysics",
    ...
    requires: config.requires ?? ["n:simulation:physics"],
    provides: [
