...
export function requiredShaderApi(engine, name, purpose) {
  const api = engine.n?.[name];
  if (!api) throw new Error(`${purpose} requires public capability ${name}.`);
  return api;
}

export function createShaderRegistryKit(config) {
  const {
    manifestId,
    id,
    domain,
  ...
  return createDomainKit({
    ...
    createApi({ baseApi, engine }) {
      const context = { engine, requiredApi: (name) => requiredShaderApi(engine, name, purpose) };
...
export default createShaderRegistryKit;
