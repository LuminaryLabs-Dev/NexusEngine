...
export function requireFiniteNumber(value, label, { minimum = -Infinity, exclusiveMinimum = false } = {}) {
...
export function normalizeProviderReceipt(value, { ready } = {}) {
  ...
  normalized.schema = normalized.schema ?? "nexusengine.physics-provider-readiness/1";
  if (normalized.schema !== "nexusengine.physics-provider-readiness/1") {
