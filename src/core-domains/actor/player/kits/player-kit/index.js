...
export function createPlayerKit(config = {}) {
  const apiName = config.apiName ?? "player";
  const baseKit = createDomainKit({
    ...config,

    manifestId: "player-authority-kit",

    id: config.id ?? "player-authority-kit",

    domainPath: config.domainPath ?? "n:actor:player",
    ...
    createApi({ engine, baseApi }) {
      ...
      const requireCharacter = (id) => {
        ...
        const character = engine.n?.character?.get?.(id);
      ...
      function getControlledCharacter(id) {
        ...
        const resolved = engine.n?.character?.resolve?.(player.characterId);
  ...
  return Object.freeze({
    ...baseKit,
    requires: [...new Set([...(baseKit.requires ?? []), "n:actor:character"])],
...
export default createPlayerKit;
