...
export function createCharacterKit(config = {}) {
  const apiName = config.apiName ?? "character";
  const baseKit = createDomainKit({
    ...config,

    manifestId: "character-registry-kit",

    id: config.id ?? "character-registry-kit",

    domainPath: config.domainPath ?? "n:actor:character",

    parentDomainPath: config.parentDomainPath ?? "n:actor",
    domain: "character",
    apiName,
    ...
    createApi({ engine, baseApi }) {
      ...
      const requireCreature = (id) => {
        const creature = engine.n?.creature?.get?.(id);
      ...
      function remove(id) {
        ...
        const referenced = engine.n?.player?.list?.().some((player) => player.characterId === key);
  ...
  return Object.freeze({
    ...baseKit,
    requires: [...new Set([...(baseKit.requires ?? []), "n:actor:creature"])],
...
export default createCharacterKit;
