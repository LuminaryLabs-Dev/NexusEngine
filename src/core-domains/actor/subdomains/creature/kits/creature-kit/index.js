...
export function createCreatureKit(config = {}) {
  const apiName = config.apiName ?? "creature";
  const baseKit = createDomainKit({
    ...config,

    manifestId: "creature-registry-kit",

    id: config.id ?? "creature-registry-kit",

    domainPath: config.domainPath ?? "n:actor:creature",
    ...
    createApi({ engine, baseApi }) {
      ...
      function remove(id) {
        ...
        const referenced = engine.n?.character?.list?.().some((character) => character.creatureId === key);
...
export default createCreatureKit;
