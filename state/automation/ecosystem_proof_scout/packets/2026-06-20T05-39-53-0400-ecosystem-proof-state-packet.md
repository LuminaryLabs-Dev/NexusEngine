...
Result: partial; core remains latest-release aligned and smoke-green, ProtoKits now matches fetched `origin/0.0.2`, but first-wave DSK ecosystem proof is still blocked by the same `nexusengine` module-source strategy across ProtoKits, Experiments targeted proof, and the public browser route.
...
- `protokits/nexus-dsk-adapter/index.js` imports `nexusengine` directly as a bare package.
...
- Local and fetched ProtoKits package metadata both report `@luminarylabs/nexusengine-protokits@0.0.2`, but neither validation path has a package/workspace/CDN/link model that resolves package `nexusengine`.
