...
## Latest branch
- NexusRealtime preflight resolved `latestReleaseBranch: 0.0.2`.
- NexusRealtime: local `0.0.2` at `2b3b66da33a2c561f0ad901604ba09404f747a82`, matching `origin/0.0.2`; ahead/behind `0/0`.
...
- `https://luminarylabs-agents.github.io/NexusRealtime-Experiments/experiments/dsk-first-wave-proof/` -> 404 by curl and visible Playwright check.
...
- Sampled stale pins with specific kit files returned 200 for `NexusRealtime@main`, `NexusRealtime@0.0.1`, `NexusRealtime-ProtoKits@main`, and `NexusRealtime-ProtoKits@0.0.1`.
- Non-file aggregate probe `NexusRealtime-ProtoKits@main/protokits/index.js` returned 404 and should not be used as a public proof URL.

## ProtoKits migration state
- Direct-import DSK adapter exists locally at `protokits/nexus-dsk-adapter/index.js` and imports bare `nexusrealtime`.
...
- Local proof HTML declares a browser import map for bare `nexusrealtime`, but maps to the sibling local source path `../../../NexusRealtime/src/index.js`.
...
- NexusRealtime core still owns the generic DSK contract and passes `domain-service-kit-smoke`.
- The first-wave DSK proof keeps runtime behavior in NexusRealtime/ProtoKits; Experiments hosts the proof page and smoke route.
