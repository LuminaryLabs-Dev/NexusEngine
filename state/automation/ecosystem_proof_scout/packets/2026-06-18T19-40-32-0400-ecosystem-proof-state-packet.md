...
- NexusRealtime: local `0.0.2` at `2b3b66da33a2c561f0ad901604ba09404f747a82`, matching `origin/0.0.2`.
...
## Commands run
- NexusRealtime: `npm run automation:preflight` -> passed; required public links OK.
...
- `https://cdn.jsdelivr.net/gh/LuminaryLabs-Dev/NexusRealtime@0.0.2/src/index.js` -> 200 by preflight and direct curl.
- `https://registry.npmjs.org/nexusrealtime` -> 404 by preflight; optional link.
- `https://luminarylabs-agents.github.io/NexusRealtime-Experiments/` -> 200.
- `https://luminarylabs-agents.github.io/NexusRealtime-Experiments/experiments/dsk-first-wave-proof/` -> 404.
- `https://raw.githubusercontent.com/LuminaryLabs-Agents/NexusRealtime-Experiments/0.0.2/experiments/dsk-first-wave-proof/index.html` -> 404.
- `https://raw.githubusercontent.com/LuminaryLabs-Agents/NexusRealtime-ProtoKits/0.0.2/docs/DSK-FIRST-WAVE-LEDGER.md` -> 404.
- Sampled stale/stable CDN pins for `NexusRealtime@main`, `NexusRealtime@0.0.1`, `NexusRealtime-ProtoKits@0.0.1`, and `NexusRealtime-ProtoKits@main` -> 200; availability is OK, branch freshness is the risk.

## ProtoKits migration state
- Direct-import DSK path exists locally through `protokits/nexus-dsk-adapter/index.js`, which imports bare `nexusrealtime`, normalizes direct-import vs injected-runtime calls, and maps first-wave kits into `defineDomainServiceKit()` when available.
...
- The proof uses an import map for bare `nexusrealtime`, imports first-wave ProtoKit N aliases, composes `createRealtimeGame()`, ticks the engine, and reads promoted APIs under `engine.n.*`.
...
- Core NexusRealtime owns the generic DSK contract under `src/domain-service-kit.js` and passes `tests/domain-service-kit-smoke.mjs`.
- The first-wave DSK proof keeps runtime behavior in NexusRealtime/ProtoKits and uses Experiments as a host/proof route.
...
- Published DSK proof route is missing: `https://luminarylabs-agents.github.io/NexusRealtime-Experiments/experiments/dsk-first-wave-proof/` -> 404.
- Raw branch proof files are missing for the sampled `0.0.2` URLs in ProtoKits and Experiments.
- Experiments still contains public URLs pinned to `NexusRealtime@main`, `NexusRealtime@0.0.1`, `NexusRealtime-ProtoKits@main`, and `NexusRealtime-ProtoKits@0.0.1`.
...
- Compatibility shims preserve old `createXKit(NexusRealtime, config)` and `engine.*` APIs, which is useful for migration but leaves two valid call paths to audit.
