# Knowledge Nodes: ecosystem proof scout 2026-06-19T16-39-19-0400

## Root Lesson
- id: ecosystem-proof-018
- statement: Core and local available-checkout DSK proof remain green, but fetched-ref and public proof are still blocked by one module-source strategy problem: ProtoKits fetched-ref proof cannot resolve package `nexusengine`, Experiments fetched-ref targeted proof cannot resolve sibling `NexusEngine`, and the public browser proof remains stuck at `Booting...` on deployed sibling module 404s; lifecycle parity rows now join hardening inventory separately from distribution proof.
- why it matters: Promotion review must keep local checkout success, latest release-ref validation, aggregate checks, targeted DSK checks, browser-complete public proof, npm availability, runtime hardening, lifecycle parity, and experience-edge proof as separate claims.

## Child Nodes
- id: ecosystem-proof-018-a
...
- id: ecosystem-proof-018-g
  parent: ecosystem-proof-018
  lesson: Lifecycle parity rows join hardening inventory but stay separate from public route loading and package validation.
  evidence: Deep bug node `2026-06-19T15-53-59-0400` and domain idea node `2026-06-19T16-01-29-0400` add held-glide receipts, terrain query/chunk parity, camera-overlay partial-start cleanup, and AR stop-state semantics.
...
- reason: Preserves local-green/public-failing proof split while refreshing current fetched-ref, Playwright, and lifecycle parity evidence.
...
- reason: Orders hardening as runtime failure-boundary, lifecycle parity, experience-edge, and public module-source proof as separate categories.
- source: state/automation/deep_bug_report_scout/knowledge_nodes/2026-06-19T15-53-59-0400-deep-bug-node.md
- relationship: extends
- reason: Adds held-input, terrain parity, and AR lifecycle cleanup rows that require fixture planning after base proof boundaries stay clear.
- source: state/automation/domain_kit_idea_expander/knowledge_nodes/2026-06-19T16-01-29-0400-domain-kit-idea-node.md
- relationship: expands
- reason: Converts lifecycle parity bugs into advisory proof planning rows until executable coverage exists.
...
- branch: lifecycle-parity-proof
  ...
  question: Which held-input receipt, terrain parity, partial-start cleanup, and stopped-session rows belong in the smallest executable fixture set?
...
- This node does not claim DSK hardening, lifecycle parity, or experience-edge blockers are fixed.
