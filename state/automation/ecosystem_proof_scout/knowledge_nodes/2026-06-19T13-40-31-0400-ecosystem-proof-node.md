...
- statement: Core and local available-checkout DSK proof remain green, but the latest proof blockers are still module-source gates: ProtoKits fetched-ref proof cannot resolve package `nexusengine`, Experiments fetched-ref targeted proof cannot resolve sibling `NexusEngine`, and public browser proof remains stuck at `Booting...` on deployed sibling module 404s.
