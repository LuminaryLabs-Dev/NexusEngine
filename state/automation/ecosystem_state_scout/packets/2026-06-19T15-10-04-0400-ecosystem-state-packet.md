...
- Disposable fetched `/Users/crimsonwheeler/Documents/GitHub/NexusEngine-ProtoKits` `origin/0.0.2`
...
- Disposable fetched `/Users/crimsonwheeler/Documents/GitHub/NexusEngine-Experiments` `origin/0.0.2`
...
- ProtoKits disposable fetched-ref validation still fails at the same package-resolution gate: `npm run check` fails during `smoke:imports` with 110 ProtoKit import failures rooted in `ERR_MODULE_NOT_FOUND` for package `nexusengine`; targeted `node tests/dsk-first-wave.test.mjs` fails for missing `nexusengine`.
